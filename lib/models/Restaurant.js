...
class Restaurant {
  ...
  static async newReview({ star_rating, restaurant_id, user_id }) {
    const { rows } = await pool.query(
      'insert into restaurants_reviews (star_rating, restaurant_id, user_id) values ($1, $2, $3) returning *;',
      [star_rating, restaurant_id, user_id]
    );
    return new Restaurant(rows[0]);
  }
}
